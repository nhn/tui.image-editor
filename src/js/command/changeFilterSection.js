...
const command = {
    name: commandNames.CHANGE_FILTER_SECTION,
    ...
    execute(graphics, id, options, isSilent) {
        ...
        return filtersectionComp.change(targetObj, options);
    },
    /**
     * @param {Graphics} graphics - Graphics instance
     * @returns {Promise}
    ...
    undo(graphics) {
        const filtersectionComp = graphics.getComponent(FILTER_SECTION);
        const {object: filtersection, options} = this.undoData;

        return filtersectionComp.change(filtersection, options);
    }
};
